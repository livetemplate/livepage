...
- **The reference is now trustworthy as generation context, and mechanically kept so** — `TestDocumentedAttributesExist` fails the build if a doc surface names an attribute nothing implements. M1 Phase 3 can wire `reference.md` into the skill's context without inheriting this class of bug.
