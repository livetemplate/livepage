...
- [ ] **Unit:** config-load tests — approved-set resolution, describes-metadata, backward-compat with a plain `tinkerdown.yaml`, override priority unchanged **when no manifest is present**.
- [ ] **Unit (the pin):** a frontmatter source **and** a frontmatter action, each sharing a name with an approved entry, do not override it; the same names override normally when no manifest exists; unapproved frontmatter names still merge (the lint, not precedence, is what flags those).
- [ ] **Integration:** load the demo manifest; assert the `Manifest` accessor returns exactly the approved sources/actions/style.
- [ ] **E2E (the missing evidence, per the Audit):** serve a page whose frontmatter shadows an approved source name and assert **at runtime** that the approved definition is what resolves — two valid, distinguishable sources so the outcome cannot be confused with "nothing rendered," which is precisely how four Audit probes failed. This is the demo the shadowing finding still lacks; it doubles as the regression test for the pin.
